...
public class DefaultRoleManager implements ExtendedRoleManager, EventSource {
	...
	if (original == null)
	    throw CodeableException.newException(RoleNotFoundException.class, 5152, role.getRoleId()); // throw
												       // new
	...
	if (!nameEquals(original, role)) {
		...
		if (checked.getRoleId() == role.getRoleId()) {
		    throw CodeableException.newException(RoleAlreadyExistsException.class, 5151); // new
												  // RoleAlreadyExistsException("Role
												  // with
												  // this
												  // name
												  // already
	...
	if (r == null)
	    throw CodeableException.newException(RoleNotFoundException.class, 5153, name);// new
	...
	if (role == null)
	    throw CodeableException.newException(RoleNotFoundException.class, 5152, roleId); // new
											     // RoleNotFoundException((new
											     // StringBuilder()).append("No
											     // role
											     // found
	...
	return g1.getName() != null && g2.getName() != null
		&& caseRoleName(g1.getName()).equals(caseRoleName(g2.getName()));
    }

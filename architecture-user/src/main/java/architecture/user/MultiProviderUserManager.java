...
public class MultiProviderUserManager implements UserManager, EventSource {
    ...
    public void setAllowApplicationUserCreation(boolean allowApplicationUserCreation) {
    ...
    public User createUser(User newUser, Company company)
	...
	if (null != user)
		...
		if (caseEmailAddress(user).equals(caseEmailAddress(newUser))) {
		    EmailAlreadyExistsException e = CodeableException.newException(EmailAlreadyExistsException.class,
			    5104, user.getUsername(), caseEmailAddress(user));
		    throw e;
		}
	    } else {
		UserAlreadyExistsException e = CodeableException.newException(UserAlreadyExistsException.class, 5105,
			user.getUsername(), caseEmailAddress(user));
	...
	User user = getUser(newUser);
	if (null != user) {
	    UserAlreadyExistsException e = CodeableException.newException(UserAlreadyExistsException.class, 5110,
		    user.getUsername());
	    log.info(e.getMessage());
	    throw e;
	}
	...
	if (template.getUserId() == -1L
		|| template.getUsername() != null && template.getUsername().equals("ANONYMOUS")) {
	...
	if (null == loadedUser) {
		...
		String message = (new StringBuilder()).append("Failed to remove application user for ").append(user)
			.append(".").toString();
	...
	/*
	 * if(null == loadedUser) log.info((new StringBuilder()).append(
	 * "Unable to load user for deletion "
	 * ).append(user.toString()).append(".").toString()); else
	 * dispatchEvent(new
	 * UserEvent(com.jivesoftware.base.event.UserEvent.Type.DELETED, new
	 * UserTemplate(loadedUser), null));
	 */
	// eventPublisher.publish(event);
    }
	...
	// dispatchEvent(new
	// UserEvent(com.jivesoftware.base.event.UserEvent.Type.MODIFIED,
	// userModel, eventParams));
	// eventPublisher.publish(event);
	return userModel;
    }
    ...
    protected void modifyUserEnabled(User user, boolean enabled)
	...
	try {
	    log.debug(user.getPassword() + "="
		    + passwordEncoder.encodePassword(passwd, saltSource.getSalt(new ExtendedUserDetailsAdaptor(user))));
    ...
    public List<User> findUsersWithGroupFilter(Company company, Group group, String nameOrEmail, int startIndex,
	    int numResults) {

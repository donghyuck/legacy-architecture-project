...
public class JdbcGroupDao extends ExtendedJdbcDaoSupport implements GroupDao {
	...
	getExtendedJdbcTemplate().update(getBoundSql("ARCHITECTURE_SECURITY.CREATE_GROUP").getSql(),
		...
		new SqlParameterValue(Types.NUMERIC, groupId), new SqlParameterValue(Types.VARCHAR, group.getName()),
	...
	try {
	    group = getExtendedJdbcTemplate().queryForObject(
		    getBoundSql("ARCHITECTURE_SECURITY.SELECT_GROUP_BY_ID").getSql(), groupMapper,
		    new SqlParameterValue(Types.NUMERIC, groupId));
	    group.setProperties(getGroupProperties(group.getGroupId()));
	} catch (IncorrectResultSizeDataAccessException e) {
	    if (e.getActualSize() > 1) {
		log.warn((new StringBuilder()).append("Multiple occurrances of the same group ID found: ")
			.append(groupId).toString());
		throw e;
	    }
	} catch (DataAccessException e) {
	    String message = (new StringBuilder()).append("Failure attempting to load group by ID : ").append(groupId)
		    .append(".").toString();
	    log.fatal(message, e);
	}
	return group;
	...
	try {
	    group = getExtendedJdbcTemplate().queryForObject(
		    getBoundSqlWithAdditionalParameter("ARCHITECTURE_SECURITY.SELECT_GROUP_BY_NAME",
			    new Boolean(caseInsensitive)).getSql(),
		    groupMapper, new SqlParameterValue(Types.VARCHAR, caseInsensitive ? name.toLowerCase() : name));

	    group.setProperties(getGroupProperties(group.getGroupId()));
	} catch (IncorrectResultSizeDataAccessException e) {
	    if (e.getActualSize() > 1) {
		log.warn((new StringBuilder()).append("Multiple occurrances of the same group ID found: ").append(name)
			.toString());
		throw e;
	    }
	} catch (DataAccessException e) {
	    String message = (new StringBuilder()).append("Failure attempting to load group by ID : ").append(name)
		    .append(".").toString();
	    log.fatal(message, e);
	}
	return group;
	...
	return getExtendedJdbcTemplate().queryForList(getBoundSql("ARCHITECTURE_SECURITY.SELECT_ALL_GROUP_ID").getSql(),
		Long.class);
    }

    public List<Long> getGroupIds(int startIndex, int numResults) {
	return getExtendedJdbcTemplate().queryScrollable(
		getBoundSql("ARCHITECTURE_SECURITY.SELECT_ALL_GROUP_ID").getSql(), startIndex, numResults,
		new Object[0], new int[0], Long.class);
    }
	...
	return getExtendedJdbcTemplate().queryForList(
		getBoundSql("ARCHITECTURE_SECURITY.SELECT_ALL_GROUP_MEMBER_ID").getSql(), new Object[] { groupId },
		new int[] { Types.NUMERIC }, Long.class);
    }

    public List<Long> getMembersIds(long groupId, int startIndex, int numResults) {
	return getExtendedJdbcTemplate().queryScrollable(
		getBoundSql("ARCHITECTURE_SECURITY.SELECT_ALL_GROUP_MEMBER_ID").getSql(), startIndex, numResults,
		new Object[] { groupId }, new int[] { Types.NUMERIC }, Long.class);
    }

    public int getGroupMemberCount(long groupId) {
	return getExtendedJdbcTemplate().queryForInt(
		getBoundSql("ARCHITECTURE_SECURITY.COUNT_ALL_GROUP_MEMBER").getSql(),
		new SqlParameterValue(Types.NUMERIC, groupId));
    }
	...
	List<Long> userIds = getExtendedJdbcTemplate().queryForList(
		getBoundSql("ARCHITECTURE_SECURITY.SELECT_USER_ID_FROM_GROUP_BY_GROUP_ID_AND_USER_ID").getSql(),
	...
	getExtendedJdbcTemplate().update(getBoundSql("ARCHITECTURE_SECURITY.ADD_GROUP_MEMBER").getSql(),
		new SqlParameterValue(Types.NUMERIC, groupId), new SqlParameterValue(Types.NUMERIC, userId));

	getExtendedJdbcTemplate().update(getBoundSql("ARCHITECTURE_SECURITY.UPDATE_GROUP_MODIFIED_DATE").getSql(),
		new SqlParameterValue(Types.DATE, new Date()), new SqlParameterValue(Types.NUMERIC, groupId));
    }
	...
	getExtendedJdbcTemplate().batchUpdate(getBoundSql("ARCHITECTURE_SECURITY.ADD_GROUP_MEMBER").getSql(),
	...
	return getExtendedJdbcTemplate().queryForList(
		getBoundSql("ARCHITECTURE_SECURITY.SELECT_GROUPS_FOR_USER").getSql(), Long.class,
		new SqlParameterValue(Types.NUMERIC, userId));
    }

...
public class ExternalJdbcUserDao extends ExtendedJdbcDaoSupport implements UserDao {
    ...
    public void setExternalUserProfileDao(ExternalUserProfileDao externalUserProfileDao) {
	...
	return getExtendedJdbcTemplate().queryForList(getSql("SELECT_USER_ID_BY_PROPERTY"),
	...
	if (user.getEmail() == null)
	    throw new IllegalArgumentException(
		    "User has no email address specified.  An email address is required to create a new user.");
	...
	try {
	    ...
	    getExtendedJdbcTemplate().update(getSql("CREATE_USER"),
		    new Object[] { userId, user.getUsername(), user.getPasswordHash(),
			    useLastNameFirstName ? null : user.getName(), user.isNameVisible() ? 1 : 0, user.getEmail(),
	...
	try {
	    ...
	    getExtendedJdbcTemplate().update(getSql("UPDATE_USER"),
		    new Object[] { useLastNameFirstName ? null : user.getName(), user.isNameVisible() ? 1 : 0,
			    ...
			    user.getUsername(), user.isExternal(), user.isFederated(),
	...
	try {
	    user = (UserTemplate) getExtendedJdbcTemplate().queryForObject(getSql("SELECT_USER_BY_USERNAME"),
		    new Object[] { username }, new int[] { Types.VARCHAR }, getExternalUserRowMapper());
	...
	try {
	    user = (UserTemplate) getExtendedJdbcTemplate().queryForObject(getSql("SELECT_USER_BY_USERNAME"),
		    new Object[] { username.toLowerCase() }, new int[] { Types.VARCHAR }, getExternalUserRowMapper());
	    user.setProfile(getUserProfile(user.getUserId()));
	    user.setProperties(getUserProperties(user.getUserId()));
	} catch (EmptyResultDataAccessException e) {
	    String message = (new StringBuilder())
	...
	getExtendedJdbcTemplate().update(getSql("DELETE_GROUP_MEMBERSHIP"), new Object[] { user.getUserId() },
		new int[] { Types.INTEGER });
	extendedPropertyDao.deleteProperties(userPropertyTableName, userPropertyPrimaryColumnName, user.getUserId());
	getExtendedJdbcTemplate().update(getSql("DELETE_USER_BY_ID"), new Object[] { user.getUserId() },
		new int[] { Types.INTEGER });
    }
	...
	List<User> users = getExtendedJdbcTemplate().query(getSql("SELECT_ALL_ENABLED_USER"),
		getExternalUserRowMapper());
	for (User user : users) {
	...
	List<User> users = getExtendedJdbcTemplate().queryScrollable(getSql("SELECT_ALL_ENABLED_USER"), startIndex,
		numResults, new Object[0], new int[0], getExternalUserRowMapper());
	for (User user : users) {
	    ((UserTemplate) user).setProfile(this.getUserProfile(user.getUserId()));
	    ((UserTemplate) user).setProperties(this.getUserProperties(user.getUserId()));
	}
	...
	List<User> users = getExtendedJdbcTemplate().query(getSql("SELECT_ALL_VISIBLE_USER"),
		getExternalUserRowMapper());
	for (User user : users) {
	    ((UserTemplate) user).setProperties(this.getUserProperties(user.getUserId()));
	}
	...
	List<User> users = getExtendedJdbcTemplate().queryScrollable(getSql("SELECT_ALL_VISIBLE_USER"), startIndex,
		numResults, new Object[0], new int[0], getExternalUserRowMapper());
	for (User user : users) {
	    ((UserTemplate) user).setProfile(this.getUserProfile(user.getUserId()));
	    ((UserTemplate) user).setProperties(this.getUserProperties(user.getUserId()));
	...
	List<User> users = getExtendedJdbcTemplate().query(getSql("SELECT_USERS_BY_EMAIL_OR_NAME"),
		getExternalUserRowMapper(), new SqlParameterValue(Types.VARCHAR, nameOrEmail),
	...
	List<User> users = getExtendedJdbcTemplate().queryScrollable(getSql("SELECT_USERS_BY_EMAIL_OR_NAME"),
		startIndex, numResults, new Object[] { nameOrEmail, nameOrEmail },
		new int[] { Types.VARCHAR, Types.VARCHAR }, getExternalUserRowMapper());
	for (User user : users) {
	    ((UserTemplate) user).setProfile(this.getUserProfile(user.getUserId()));
	    ((UserTemplate) user).setProperties(this.getUserProperties(user.getUserId()));
	}
	...
	List<User> users = getExtendedJdbcTemplate().queryScrollable(getSql("SELECT_ALL_COMPANY_VISIBLE_USER"),
		startIndex, numResults, new Object[] { company.getCompanyId() }, new int[] { Types.INTEGER },
		userMapper);
	for (User user : users) {
	    try {
		((UserTemplate) user).setCompany(CompanyUtils.getCompany(user.getCompanyId()));
	...
	List<User> users = getExtendedJdbcTemplate().query(getSql("SELECT_COMPANY_USERS_BY_EMAIL_OR_NAME"), userMapper,
	...
	List<User> users = getExtendedJdbcTemplate().queryScrollable(getSql("SELECT_COMPANY_USERS_BY_EMAIL_OR_NAME"),
		startIndex, numResults, new Object[] { company.getCompanyId(), nameOrEmail, nameOrEmail },

...
public class JdbcUserDao extends ExtendedJdbcDaoSupport implements UserDao {
	...
	try {
	    ...
	    getExtendedJdbcTemplate().update(getBoundSql("ARCHITECTURE_SECURITY.CREATE_USER").getSql(),
		    new Object[] { user.getCompanyId(), userId, user.getUsername(), user.getPasswordHash(),
			    useLastNameFirstName ? null : user.getName(), user.isNameVisible() ? 1 : 0,
			    useLastNameFirstName ? user.getFirstName() : null,
			    useLastNameFirstName ? user.getLastName() : null, user.getEmail(),
		    ...
		    new int[] { Types.NUMERIC, Types.NUMERIC, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR,
			    Types.NUMERIC, Types.VARCHAR, Types.VARCHAR, Types.VARCHAR, Types.NUMERIC, Types.TIMESTAMP,

			    Types.TIMESTAMP, Types.NUMERIC, Types.NUMERIC, Types.NUMERIC, Types.NUMERIC, Types.NUMERIC,
	...
	try {
	    ...
	    getExtendedJdbcTemplate().update(getBoundSql("ARCHITECTURE_SECURITY.UPDATE_USER").getSql(),
		    new Object[] { useLastNameFirstName ? null : user.getName(),
			    useLastNameFirstName ? user.getFirstName() : null,
			    useLastNameFirstName ? user.getLastName() : null, user.isNameVisible() ? 1 : 0,
	...
	try {
	    user = getExtendedJdbcTemplate().queryForObject(
		    ...
		    new int[] { Types.VARCHAR }, userMapper);
	    try {
		user.setCompany(CompanyUtils.getCompany(user.getCompanyId()));
	    } catch (CompanyNotFoundException e) {
	    }
	...
	try {
	    user = getExtendedJdbcTemplate().queryForObject(
		    ...
		    new Object[] { username.toLowerCase() }, new int[] { Types.VARCHAR }, userMapper);
	    try {
		user.setCompany(CompanyUtils.getCompany(user.getCompanyId()));
	    } catch (CompanyNotFoundException e) {
	    }
	...
	if (null != email) {
		...
		UserTemplate user = getExtendedJdbcTemplate().queryForObject(
			getBoundSql("ARCHITECTURE_SECURITY.SELECT_USER_BY_EMAIL").getSql(), userMapper,
	...
	try {
	    user = getExtendedJdbcTemplate().queryForObject(
		    getBoundSql("ARCHITECTURE_SECURITY.SELECT_USER_BY_ID").getSql(), userMapper,
	...
	List<User> users = getExtendedJdbcTemplate()
		.query(getBoundSql("ARCHITECTURE_SECURITY.SELECT_ALL_ENABLED_USER").getSql(), userMapper2);
	for (User user : users) {
	    try {
		((UserTemplate) user).setCompany(CompanyUtils.getCompany(user.getCompanyId()));
	    } catch (CompanyNotFoundException e) {
	...
	List<User> users = getExtendedJdbcTemplate().queryScrollable(
		...
		new Object[0], new int[0], userMapper2);
	for (User user : users) {
	    try {
		((UserTemplate) user).setCompany(CompanyUtils.getCompany(user.getCompanyId()));
	    } catch (CompanyNotFoundException e) {
	...
	return getExtendedJdbcTemplate()
		.queryForInt(getBoundSql("ARCHITECTURE_SECURITY.COUNT_AUTHENTICATED_USER").getSql());
    }

    public int getRecentUserCount(Date date) {
	return getExtendedJdbcTemplate().queryForInt(getBoundSql("ARCHITECTURE_SECURITY.COUNT_RECENT_USER").getSql(),
		new Object[] { date }, new int[] { Types.DATE });
    }

    public List<User> getAllUsers() {
	List<User> users = getExtendedJdbcTemplate()
		.query(getBoundSql("ARCHITECTURE_SECURITY.SELECT_ALL_VISIBLE_USER").getSql(), userMapper2);
	for (User user : users) {
	    try {
		((UserTemplate) user).setCompany(CompanyUtils.getCompany(user.getCompanyId()));
	    } catch (CompanyNotFoundException e) {
	...
	List<User> users = getExtendedJdbcTemplate().query(
		getBoundSql("ARCHITECTURE_SECURITY.SELECT_USERS_BY_EMAIL_OR_NAME").getSql(), userMapper2,
	...
	List<User> users = getExtendedJdbcTemplate().queryScrollable(
		...
		new Object[] { nameOrEmail, nameOrEmail }, new int[] { Types.VARCHAR, Types.VARCHAR }, userMapper2);
	for (User user : users) {
	    try {
		((UserTemplate) user).setCompany(CompanyUtils.getCompany(user.getCompanyId()));
	    } catch (CompanyNotFoundException e) {
	...
	return getExtendedJdbcTemplate().queryForInt(getBoundSql("ARCHITECTURE_SECURITY.COUNT_COMPANY_USERS").getSql(),
		new SqlParameterValue(Types.INTEGER, company.getCompanyId()));
    }

    public List<User> getUsers(Company company) {
	List<User> users = getExtendedJdbcTemplate()
		.query(getBoundSql("ARCHITECTURE_SECURITY.SELECT_ALL_COMPANY_VISIBLE_USER").getSql(), userMapper2);
	for (User user : users) {
	    try {
		((UserTemplate) user).setCompany(CompanyUtils.getCompany(user.getCompanyId()));
	    } catch (CompanyNotFoundException e) {
	...
	List<User> users = getExtendedJdbcTemplate().queryScrollable(
		...
		new Object[] { company.getCompanyId() }, new int[] { Types.INTEGER }, userMapper2);
	for (User user : users) {
	    try {
		((UserTemplate) user).setCompany(CompanyUtils.getCompany(user.getCompanyId()));
	    } catch (CompanyNotFoundException e) {
	...
	List<User> users = getExtendedJdbcTemplate().query(
		getBoundSql("ARCHITECTURE_SECURITY.SELECT_COMPANY_USERS_BY_EMAIL_OR_NAME").getSql(), userMapper2,
		new SqlParameterValue(Types.INTEGER, company.getCompanyId()),
		new SqlParameterValue(Types.VARCHAR, nameOrEmail), new SqlParameterValue(Types.VARCHAR, nameOrEmail));
	for (User user : users) {
	    try {
	...
	return getExtendedJdbcTemplate().queryForList(
		getBoundSql("ARCHITECTURE_SECURITY.SELECT_COMPANY_USER_IDS_BY_EMAIL_OR_NAME_WITH_GROUP_FILTER")
			.getSql(),
	...
	return getExtendedJdbcTemplate().queryForObject(
		...
		Integer.class, new SqlParameterValue(Types.INTEGER, company.getCompanyId()),
		new SqlParameterValue(Types.VARCHAR, nameOrEmail), new SqlParameterValue(Types.VARCHAR, nameOrEmail),
		new SqlParameterValue(Types.INTEGER, group.getGroupId()));
    }

...
public class JdbcCompanyDao extends ExtendedJdbcDaoSupport implements CompanyDao {
	...
	try {
		...
		log.warn((new StringBuilder()).append("Multiple occurrances of the same company domainName found: ")
	...
	try {
	    ...
	    if (e.getActualSize() > 1) {
		log.warn((new StringBuilder()).append("Multiple occurrances of the same company name found: ")
	...
	try {
	    ...
	    if (e.getActualSize() > 1) {
		log.warn((new StringBuilder()).append("Multiple occurrances of the same company ID found: ")
			.append(companyId).toString());
		throw e;
	    }
	} catch (DataAccessException e) {
	    String message = (new StringBuilder()).append("Failure attempting to load company by ID : ")
		    .append(companyId).append(".").toString();
	    log.fatal(message, e);
	}
	return company;
    }
	...
	return getExtendedJdbcTemplate().queryScrollable(
		getBoundSql("ARCHITECTURE_SECURITY.SELECT_ALL_COMPANY_IDS").getSql(), startIndex, numResults,
	...
	extendedPropertyDao.updateProperties(companyPropertyTableName, companyPropertyPrimaryColumnName, companyId,
		props);
    }
	...
	return getExtendedJdbcTemplate().queryForList(
		getBoundSql("ARCHITECTURE_SECURITY.SELECT_COMPANY_GROUP_IDS").getSql(), Long.class,
		new SqlParameterValue(Types.NUMERIC, companyId));
    }

    public List<Long> getAllCompanyIds() {
	return getExtendedJdbcTemplate()
		.queryForList(getBoundSql("ARCHITECTURE_SECURITY.SELECT_ALL_COMPANY_IDS").getSql(), Long.class);
    }

    public List<Long> getCompanyGroupIds(long companyId, int startIndex, int numResults) {
	return getExtendedJdbcTemplate().queryScrollable(
		getBoundSql("ARCHITECTURE_SECURITY.SELECT_COMPANY_GROUP_IDS").getSql(), startIndex, numResults,
		new Object[] { companyId }, new int[] { Types.INTEGER }, Long.class);
    }
	...
	return getExtendedJdbcTemplate().query(
		...
		new RowMapper<DomainMatcher>() {
		    public DomainMatcher mapRow(ResultSet rs, int rowNum) throws SQLException {
			DomainMatcher matcher = new DomainMatcher(1, rs.getLong("COMPANY_ID"),

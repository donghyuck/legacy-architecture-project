...
public class SecurityHelperImpl implements Implementation {
	...
	/*
	 * try { Authentication authen = getAuthentication(); Object obj =
	 * authen.getPrincipal(); if ( obj instanceof AuthToken ) return
	 * (AuthToken)obj; } catch (Exception ignore) { } return new
	 * AnonymousUser();
	 */
	...
	try {
	    /*
	     * Authentication authen = getAuthentication(); Object obj =
	     * authen.getPrincipal(); if ( obj instanceof ExtendedUserDetails )
	 ...
	 * "Authentication attempt by unregistered user '%s'.", new Object[] {
	 * username })); throw new ActivationRequiredException(
	 * "Account not yet activated."); } if(status ==
	 * User.Status.awaiting_moderation || status == User.Status.rejected) {
	 * log.info(String.format(
	 * "Authentication attempt by unapproved user '%s'.", new Object[] {
	 * username })); throw new ApprovalRequiredException(
	 * "Account not yet approved."); } if(status !=
	 * com.jivesoftware.base.User.Status.registered) {
	 * log.info(String.format(
	 * "Authentication attempt by user, '%s', not in registered state, their current state is '%s'."
	 * , new Object[] { username, status })); throw new
	 * UserNotRegisteredException("User not properly registered"); } else {
	 * return; }
	 */
    }
}

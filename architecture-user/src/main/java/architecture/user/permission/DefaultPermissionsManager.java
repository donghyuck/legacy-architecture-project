...
public class DefaultPermissionsManager implements PermissionsManager, EventSource {
	...
	public PermissionsCacheEntry(long anonymousAdditivePerms, long anonymousNegativePerms,
		long registeredAdditivePerms, long registeredNegativePerms, long[] additiveUserIds,
		long[] negativeUserIds, long[] additiveUserPerms, long[] negativeUserPerms, long[] additiveGroupIds,
		long[] negativeGroupIds, long[] additiveGroupPerms, long[] negativeGroupPerms) {
	    if (additiveUserIds.length != additiveUserPerms.length || negativeUserIds.length != negativeUserPerms.length
	...
	public List<Long> getUsersWithPerm(PermissionType permissionType, long permission) {
		...
		for (int i = 0; i < additiveUserPerms.length; i++) {
		    log.debug(
			    "additiveUserPerms[i] & permission) != 0L-" + ((additiveUserPerms[i] & permission) != 0L));
	...
	return LockUtils.intern(
		new StringBuilder().append("perms-cache-").append(objectType).append("-").append(objectId).toString());
    }

    public static int toDbFormat(long permission) {
	if (permission < 1L)
	    throw new IllegalArgumentException(
		    new StringBuilder().append("Permission value negative : ").append(permission).toString());
    ...
    public void addGroupPermission(int objectType, long objectId, Group group, PermissionType permissionType,
	    long permission) {
    ...
    @Transactional(readOnly = false, propagation = Propagation.REQUIRES_NEW)
    public void addUserPermission(int objectType, long objectId, User user, PermissionType permissionType,
	    long permission) {
    ...
    protected void addUserPermission(int objectType, long objectId, long userId, PermissionType permissionType,
	    long permission) {
	int permissionToUse = toDbFormat(permission);
    ...
    public boolean anonymousUserHasPermission(int objectType, long objectId, PermissionType permissionType,
	    long permission) {
	boolean flag = (getPermissionsCacheEntry(objectType, objectId).getAnonymousPerms(permissionType)
		& permission) != 0L;
	return flag;
    }
	...
	if (getPermissionMask(key) != -1L) {
		...
		throw new IllegalStateException(new StringBuilder()
			.append("There are no more free permissions mask for ").append(key).toString());
    ...
    public int getoupsWithPermissionCount(int objectType, long objectId, PermissionType permissionType,
	    long permission) {
	...
	if (bundle == null) {
		...
		for (Perm perm : userPerms) {
		    ...
		    log.debug(new StringBuilder("permission:").append(permission).append("(")
			    .append(perm.getPermission()).append(")").append(",type=").append(perm.getType())
			    .append(", userId=").append(perm.getObjectId()));
		...
		for (Perm perm : groupPerms) {
		    ...
		    log.debug(new StringBuilder("permission:").append(permission).append("(")
			    .append(perm.getPermission()).append(")").append(",type=").append(perm.getType())
			    .append(", groupId=").append(perm.getObjectId()));
		...
		bundle = new PermissionsCacheEntry(anonymousAdditivePerms, anonymousNegativePerms,
			registeredAdditivePerms, registeredNegativePerms, additiveUserIds.toArray(),
			negativeUserIds.toArray(), additiveUserPerms.toArray(), negativeUserPerms.toArray(),
			additiveGroupIds.toArray(), negativeGroupIds.toArray(), additiveGroupPerms.toArray(),
			negativeGroupPerms.toArray());
    ...
    public List<Group> groupsWithPermission(int objectType, long objectId, PermissionType permissionType,
	    long permission) {

	List<Long> groupIds = getPermissionsCacheEntry(objectType, objectId).getGroupsWithPerm(permissionType,
		permission);
    ...
    public boolean registeredUserHasPermission(int objectType, long objectId, PermissionType permissionType,
	    long permission) {
	boolean flag = (getPermissionsCacheEntry(objectType, objectId).getRegisteredPerms(permissionType)
		& permission) != 0L;
	return flag;
    }
    ...
    @Transactional(readOnly = false, propagation = Propagation.REQUIRES_NEW)
    public void removeGroupPermission(int objectType, long objectId, Group group, PermissionType permissionType,
	    long permission) {
	int permissionToUse = toDbFormat(permission);
    ...
    @Transactional(readOnly = false, propagation = Propagation.REQUIRES_NEW)
    public void removeRegisteredUserPermission(int objectType, long objectId, PermissionType permissionType,
	    long permission) {
	removeUserPermission(objectType, objectId, 0L, permissionType, permission);
    }

    protected void removeUserPermission(int objectType, long objectId, long userId, PermissionType permissionType,
	    long permission) {
	int permissionToUse = toDbFormat(permission);
    ...
    @Transactional(readOnly = false, propagation = Propagation.REQUIRES_NEW)
    public void removeUserPermission(int objectType, long objectId, User user, PermissionType permissionType,
	    long permission) {
	int permissionToUse = toDbFormat(permission);
    ...
    public List<User> usersWithPermission(int objectType, long objectId, PermissionType permissionType,
	    long permission) {
    ...
    public List<Group> grousWithPermission(int objectType, long objectId, PermissionType permissionType,
	    long permission) {
	List<Long> groupIds = getPermissionsCacheEntry(objectType, objectId).getGroupsWithPerm(permissionType,
		permission);
	List<Group> groups = new ArrayList<Group>(groupIds.size());
    ...
    public int groupsWithPermissionCount(int objectType, long objectId, PermissionType permissionType,
	    long permission) {
	return getPermissionsCacheEntry(objectType, objectId).getGroupsWithPerm(permissionType, permission).size();
    }
}
...
public class JdbcPermissionsDao extends ExtendedJdbcDaoSupport implements PermissionsDao {
	...
	getExtendedJdbcTemplate().update(getBoundSql("ARCHITECTURE_SECURITY.ADD_GROUP_PERM").getSql(),
		...
		new SqlParameterValue(Types.NUMERIC, permissionType), new SqlParameterValue(Types.NUMERIC, permission));
    }
	...
	return getExtendedJdbcTemplate().queryForList(
		getBoundSql("ARCHITECTURE_SECURITY.OBJECT_IDS_WITH_USER_PERMS").getSql(), Long.class,
		new SqlParameterValue(Types.NUMERIC, objectType));
    }

    public List<Long> getObjectIdsWithGroupPerms(int objectType) {
	return getExtendedJdbcTemplate().queryForList(
		getBoundSql("ARCHITECTURE_SECURITY.OBJECT_IDS_WITH_GROUP_PERMS").getSql(), Long.class,
		new SqlParameterValue(Types.NUMERIC, objectType));
    }
	...
	return getExtendedJdbcTemplate().query(getBoundSql("ARCHITECTURE_SECURITY.SELECT_USER_PERMS").getSql(),
		permRowMapper, new SqlParameterValue(Types.NUMERIC, objectType),
	...
	getExtendedJdbcTemplate().update(getBoundSql("ARCHITECTURE_SECURITY.ADD_USER_PERM").getSql(),
		new SqlParameterValue(Types.NUMERIC, objectType), new SqlParameterValue(Types.NUMERIC, objectId),
		new SqlParameterValue(Types.NUMERIC, userId), new SqlParameterValue(Types.NUMERIC, permissionType),
	...
	getExtendedJdbcTemplate().update(getBoundSql("ARCHITECTURE_SECURITY.CREATE_PERM_MASK").getSql(),
		new SqlParameterValue(Types.VARCHAR, name), new SqlParameterValue(Types.INTEGER, mask));
	...
	try {
	    return getExtendedJdbcTemplate().queryForObject(
		    getBoundSql("ARCHITECTURE_SECURITY.SELECT_PERM_MASK_BY_NAME").getSql(), Integer.class,
		    new SqlParameterValue(Types.VARCHAR, name));
	} catch (EmptyResultDataAccessException e) {
	    log.debug(
		    (new StringBuilder()).append("No mask found for permission '").append(name).append("'").toString());
	    return -1;
	}
    }

    public List<PermissionMask> getPermissionsMask() {
	return getExtendedJdbcTemplate().query(getBoundSql("ARCHITECTURE_SECURITY.SELECT_ALL_PERM_MASK").getSql(),
		permMaskRowMapper);
    }
}

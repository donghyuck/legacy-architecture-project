...
<head>
		...
		function displayPhotoPanel(){					
			...
			if( $("#" + renderToString ).length == 0  ){			
				var grid_col_size = $("#personalized-area").data("sizePlaceHolder");<#ftl encoding="UTF-8"/>
<html decorator="homepage">
<head>
		<title><#if action.webSite ?? >${action.webSite.displayName }<#else>::</#if></title>
		<#compress>		
			...
			<div id="personalized-area" class="row blank-top-10">			
				<div id="announce-panel" class="custom-panels-group col-sm-6" style="display:none;">	
					...
					<div class="alert alert-info alert-dismissable">
						 ...
						 <p><a href="${request.contextPath}/main.do?view=manage" class="btn btn-info btn-sm">My 웹사이트 이동</a></p>
					</div>
					</#if>											

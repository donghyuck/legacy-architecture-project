...
public class ExtClassLoader extends URLClassLoader {
    ...
    public void addNativePath(String path)
    {
        ...
        } else
        {
            ...
            for(StringTokenizer stringtokenizer = new StringTokenizer(path, File.pathSeparator); stringtokenizer.hasMoreTokens();)
            {
                ...
                //if(WSLauncher.debug)
                    if(exception != null)
                    	System.out.println("An error occurred adding native path \"" + (file != null ? file.toString() : "null") + "\": " + exception);
                    else
                    if(!file.exists())
                    	System.out.println("Native path \"" + file.getPath() + "\" does not exist! Specify a directory containing a native library.");
                    else
                    if(!file.isDirectory())
                    {
                    	System.out.println("Native path \"" + file.getPath() + "\" is not a directory! Specify a directory containing a native library.");
                    } else
                    {
                        String as[] = file.list();
                        if(as == null || as.length == 0)
                        {
                        	System.out.println("Native path \"" + file.getPath() + "\" is empty! Specify a directory containing a native library.");
                        } else
                        {
                            boolean flag = false;
                            boolean flag1 = false;
                            ...
                            if(!flag)
                            	System.out.println("Native path \"" + file.getPath() + "\" contains no native libraries! Specify a directory containing a native library.");
                            if(flag1)
                            	System.out.println("Native path \"" + file.getPath() + "\" contains JAR/Zip/class files. Should path be in classpath also/instead?");
                        }
                    }
                file = null;
                exception = null;
    ...
    {
        /*if(WSLauncher.debug)
            WSLauncher.out.println("looking for library on nativepath: " + s);*/
        String s1 = findLibrary0(name, nativePaths);
        if(s1 == null)
        {
            /*if(WSLauncher.debug)
                WSLauncher.out.println("looking for library on classpath: " + s);*/

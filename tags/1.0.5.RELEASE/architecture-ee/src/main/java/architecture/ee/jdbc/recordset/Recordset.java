 ...
 * It is serializable, so it can be safely used in HTTP Sessions, JNDI, etc. <br>
 * <br>
...
public class Recordset implements Serializable {
	...
	public Recordset getPage(int p) throws Throwable {
		...
		if (p < 1 || p > _pageCount)
			throw new Throwable("Invalid page number: " + p
					+ " - the Recordset contains " + _pageCount + " pages.");
		...
		// copy metadata
		HashMap<String, RecordsetField> newFields = new HashMap<String, RecordsetField>(
				_fields.keySet().size());
		...
		if (value != null) {
			...
			case java.sql.Types.DATE:
				if (!(value instanceof java.util.Date))
					throw new RecordsetException("Invalid data type of field: "
							+ fieldName
	...
	public String toString() {
		...
		while (i.hasNext()) {
			...
			pw.println(f.getName() + "|" + f.getSqlTypeName() + "|"
					+ f.getType());
		}
	...
	class Comp implements Serializable, Comparator<Record> {
		...
		public Comp(String colName, String sortMode) throws Throwable {
			...
			if (!containsField(colName))
				throw new Throwable(
						"Invalid column name passed to sort() method: "
								+ colName);
		}
		...
		public int compare(Record r1, Record r2) {
			...
			try {
				if (r1.getFieldValue(_sortCol) == null) {
					...
					Comparable<Object> x1 = (Comparable<Object>) r1
							.getFieldValue(_sortCol);
	 ...
	 * the columns: name, typename, typeid. "typeid" is the java.sql.Type value<br>
	 ...
	 * @throws Throwable
	 */
	public int findRecord(String colName, java.util.Date value)
			throws Throwable {
		int rc = -1;

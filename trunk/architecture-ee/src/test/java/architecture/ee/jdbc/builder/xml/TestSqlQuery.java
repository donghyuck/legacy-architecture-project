...
public class TestSqlQuery {
	...
	public void testQueryScrollableForList()throws Exception{
		...
		List<String> list = query.reset().setMaxResults(10).queryForList("COMMON.SELECT_TABLE_NAMES", String.class);
		for( String tablename : list){
			Integer count = query.reset().setAdditionalParameter("TABLE_NAME", tablename).queryForObject("COMMON.SELECT_COUNT_FROM_TABLE", Integer.class);
	...
	public void testQueryForObject()throws Exception{		
		...
		for( String tablename : list){
			Integer count = query.reset().setAdditionalParameter("TABLE_NAME", tablename).queryForObject("COMMON.SELECT_COUNT_FROM_TABLE", Integer.class);
			if( count > 0 ){
			  Map row = query.reset().setAdditionalParameter("TABLE_NAME", tablename).queryForMap("COMMON.SELECT_ALL_FROM_TABLE");
			  System.out.println( tablename + "=" + row.size() ) ;
			}
		}	

		for( String tablename : list){
			Integer count = (Integer)query.setStatement("COMMON.SELECT_COUNT_FROM_TABLE").setAdditionalParameter("TABLE_NAME", tablename).setReturnType(Integer.class).uniqueResult();
			if( count > 0 ){
			Map row = (Map)query.setStatement("COMMON.SELECT_ALL_FROM_TABLE").setAdditionalParameter("TABLE_NAME", tablename).setReturnType(Map.class).uniqueResult();
			  System.out.println( tablename + "=" + row.size() ) ;
			}
		}
	...
	public void testQueryForListWithParams()throws Exception{
        ...
        w.start();
		List<String> list = query.reset().queryForList("COMMON.SELECT_TZ_TUTORMONITOR_BY_YEAR", new String[]{"2010"}, new int[]{Types.VARCHAR}, String.class);
		w.stop();
		System.out.println( list.size() +  " time:" + w.getTime()) ;
		list.clear();
        w.reset();
		w.start();
        list = query.setStatement("COMMON.SELECT_TZ_TUTORMONITOR_BY_YEAR").setReturnType(String.class).setString("2010").list() ;
		w.stop();
		System.out.println( list.size() + " time:" + w.getTime()) ;		
	...
	public void testList()throws Exception{		
		...
		StopWatch w = new StopWatch();
		w.start();
		List<String> list2 =query.queryForList("COMMON.SELECT_TABLE_NAMES", String.class);
		for( String tablename : list2){
			query.setAdditionalParameter("TABLE_NAME", tablename).queryForList("COMMON.SELECT_ALL_FROM_TABLE");
		...
		w.reset();
        w.start();
		List<String> list = query.setStatement("COMMON.SELECT_TABLE_NAMES").setReturnType(String.class).list();		
		for( String tablename : list){
			List <Map<String, Object>> row = query.setStatement("COMMON.SELECT_ALL_FROM_TABLE").setReturnType(Map.class).setAdditionalParameter("TABLE_NAME", tablename).list();			
		}
		w.stop();
		System.out.println( list.size() + " time:" + w.getTime()) ;		
		System.out.println(

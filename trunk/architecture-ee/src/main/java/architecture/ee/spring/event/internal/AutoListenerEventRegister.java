package architecture.ee.spring.event.internal;
...
public class AutoListenerEventRegister implements BeanFactoryPostProcessor {
	...
	private void processEventSources(ConfigurableListableBeanFactory beanFactory) {
		Map<String, EventSource> sources = beanFactory
				.getBeansOfType(EventSource.class);
		for (Entry<String, EventSource> entry : sources.entrySet()) {
			...
			logger.info((new StringBuilder()).append("Registering bean \"")
					.append(eventSourceID).append("\" as an EventSource")
					.toString());

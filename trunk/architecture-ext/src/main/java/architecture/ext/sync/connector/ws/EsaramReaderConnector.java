...
public class EsaramReaderConnector implements ReadConnector  {
	...
	private SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddHHmmssSSS", Locale.KOREA);
	
	private boolean usingGpki = true;
	
	private EsaramHttpClient httpClient = new EsaramHttpClient() ;
		
	private EsaramGpkiService gpkiService = new EsaramGpkiService();	
	...
	public boolean isUsingGpki() {
		return usingGpki;
	}

	public void setUsingGpki(boolean usingGpki) {
		this.usingGpki = usingGpki;
	}
	...
	public Object pull(Context context) {
		...
		log.info("XML REQUEST :" + requestXml );
		...
		if(usingGpki){
			try {
				...
				//암호화
				byte[] encrypted 	= gpkiService.encrypt(original.getBytes(httpClient.getCharset()), gpkiService.getTargetServerIdList());
				//전자서명
		...
		try {
			...
			if (responseStr.indexOf("Fault>") > 0) {
				...
				if( usingGpki ){
					...
					log.debug( "ERROR XML RESPONSE : " + responseStr );
				}		

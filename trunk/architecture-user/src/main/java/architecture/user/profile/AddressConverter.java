...
public class AddressConverter extends BeanPropsExternalMapper
    implements TypeConverter
{
    ...
    public Address convertFromString(String s) throws TypeConverter.ConversionException
    {
        ...
        } else
        {
            do
            {
                ...
                if(labelAndVal.length == 2 && !StringUtils.isEmpty(labelAndVal[0]) && !StringUtils.isEmpty(labelAndVal[1]))
    ...
    {
        return address.getStreet1() != null || address.getStreet2() != null || address.getCity() != null || address.getStateOrProvince() != null || address.getPostalCode() != null || address.getCountry() != null;
    }
    ...
    public void setField(Address address, String field)
    {
        ...
        if(labelAndVal.length != 2)
            throw new IllegalArgumentException((new StringBuilder()).append(field).append(" is not of the form label:value").toString());
        ...
        if(STREET_2.equals(label))
            address.setStreet2(value);
        else
        if(CITY.equals(label))

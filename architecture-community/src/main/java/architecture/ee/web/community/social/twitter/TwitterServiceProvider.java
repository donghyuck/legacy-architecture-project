...
public class TwitterServiceProvider extends AbstractSocialServiceProvider {
	...
	super(new ServiceBuilder().provider(SSLTwitterApi.class).apiKey(clientId).apiSecret(clientSecret).debug()
		.build());
    }

    public TwitterServiceProvider(String clientId, String clientSecret, String callbackUrl) {
	super(new ServiceBuilder().provider(SSLTwitterApi.class).apiKey(clientId).apiSecret(clientSecret)

...
/**
 * 
 * OAuth Consumer Key: VdE7COcqx3c8qiXg45AowMPausYVvYDpBSIrx1jI6gPj1X5V8T Secret
 * Key: SILFKJbnYTuFLWVhCnKiD1wpd4T6ahVC0HXTemu1AqT3X7iL0r
 * 
 * @author donghyuck
 *
 */
public class TumblrServiceProvider extends AbstractSocialServiceProvider {
	...
	super(new ServiceBuilder().provider(TumblrApi.class).apiKey(consumerKey).apiSecret(consumerSecret).debug()
		.build());
	apiKey = consumerKey;
    }

    public TumblrServiceProvider(String consumerKey, String consumerSecret, String callbackUrl) {
	super(new ServiceBuilder().provider(TumblrApi.class).apiKey(consumerKey).apiSecret(consumerSecret)

...
public class JdbcCommentDao extends ExtendedJdbcDaoSupport implements CommentDao {
	...
	public Comment mapRow(ResultSet rs, int rowNum) throws SQLException {
	    ...
	    comment.setStatus(ContentObject.Status.valueOf(Integer.valueOf(rs.getInt("STATUS"))));
	    return comment;
	}
    };
    ...
    /**
     * @param commentPropertyTableName
     *            설정할 commentPropertyTableName
     */
    ...
    /**
     * @param commentPropertyPrimaryColumnName
     *            설정할 commentPropertyPrimaryColumnName
     */
    public void setCommentPropertyPrimaryColumnName(String commentPropertyPrimaryColumnName) {
	...
	extendedPropertyDao.updateProperties(commentPropertyTableName, commentPropertyPrimaryColumnName, commentId,
		props);
    }
    ...
    /**
     * COMMENT_ID OBJECT_TYPE OBJECT_ID PARENT_COMMENT_ID PARENT_OBJECT_TYPE
     * PARENT_OBJECT_ID USER_ID NAME EMAIL URL IP BODY STATUS CREATION_DATE
     * MODIFIED_DATE
     */
    ...
    /*
     * 
     * PARENT_COMMENT_ID = ?, NAME = ?, EMAIL = ?, URL = ?, IP = ?, BODY = ?,
	...
	try {
	    Comment c = getExtendedJdbcTemplate().queryForObject(
		    getBoundSql("ARCHITECTURE_COMMUNITY.SELECT_COMMENT_BY_ID").getSql(), commentMapper,
	...
	getExtendedJdbcTemplate().query(getBoundSql("ARCHITECTURE_COMMUNITY.SELECT_ROOT_COMMENT").getSql(),
	...
	getExtendedJdbcTemplate().query(getBoundSql("ARCHITECTURE_COMMUNITY.SELECT_CHILD_COMMENT").getSql(),
		new RowCallbackHandler() {
		    public void processRow(ResultSet rs) throws SQLException {
			// while( rs.next() ){
			long commentId = rs.getLong(1);

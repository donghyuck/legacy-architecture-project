...
public class JdbcLogoImageDao extends ExtendedJdbcDaoSupport implements LogoImageDao {
	...
	getExtendedJdbcTemplate().update(getBoundSql("ARCHITECTURE_COMMUNITY.UPDATE_LOGO_IMAGE").getSql(),
		new SqlParameterValue(Types.NUMERIC, logoImage.getObjectType()),
		new SqlParameterValue(Types.NUMERIC, logoImage.getObjectId()),
		new SqlParameterValue(Types.NUMERIC, logoImage.isPrimary() ? 1 : 0),
		new SqlParameterValue(Types.VARCHAR, logoImage.getFilename()),
	...
	getExtendedJdbcTemplate().update(getBoundSql("ARCHITECTURE_COMMUNITY.DELETE_LOGO_IMAGE_DATA_BY_ID").getSql(),
		new SqlParameterValue(Types.NUMERIC, logoImage.getLogoId()));
	if (getExtendedJdbcTemplate().getDatabaseType() == DatabaseType.oracle) {
	    getExtendedJdbcTemplate().update(
		    getBoundSql("ARCHITECTURE_COMMUNITY.INSERT_EMPTY_LOGO_IMAGE_DATA").getSql(),
		    new SqlParameterValue(Types.NUMERIC, logoImage.getLogoId()));
	    getExtendedJdbcTemplate().update(getBoundSql("ARCHITECTURE_COMMUNITY.UPDATE_LOGO_IMAGE_DATA").getSql(),
	...
	getExtendedJdbcTemplate().update(getBoundSql("ARCHITECTURE_COMMUNITY.DELETE_LOGO_IMAGE_BY_ID").getSql(),
		new SqlParameterValue(Types.NUMERIC, logoImage.getLogoId()));
	getExtendedJdbcTemplate().update(getBoundSql("ARCHITECTURE_COMMUNITY.DELETE_LOGO_IMAGE_DATA_BY_ID").getSql(),
		new SqlParameterValue(Types.NUMERIC, logoImage.getLogoId()));
    }
	...
	return getExtendedJdbcTemplate().queryForObject(
		...
		SqlQueryHelper.getInputStreamRowMapper(), new SqlParameterValue(Types.NUMERIC, logoImage.getLogoId()));
    }
	...
	try {
	    return getExtendedJdbcTemplate().queryForLong(
		    getBoundSql("ARCHITECTURE_COMMUNITY.SELECT_PRIMARY_LOGO_IMAGE_ID_BY_OBJECT_TYPE_AND_OBJECT_ID")
			    .getSql(),
	...
	try {
	    return getExtendedJdbcTemplate().queryForObject(
		    getBoundSql("ARCHITECTURE_COMMUNITY.SELECT_LOGO_IMAGE_BY_ID").getSql(), logoMapper,

...
public class JdbcPageVersionDao extends ExtendedJdbcDaoSupport implements PageVersionDao {
	...
	if (pageVersion.getPageState() == PageState.PUBLISHED) {
	    ...
	    getExtendedJdbcTemplate().update(getBoundSql("ARCHITECTURE_COMMUNITY.UPDATE_PAGE_STATE").getSql(),
	...
	return getExtendedJdbcTemplate().queryForObject(
		getBoundSql("ARCHITECTURE_COMMUNITY.SELECT_PAGE_BY_ID_AND_VERSION").getSql(), pageVersionMapper,
	...
	return getExtendedJdbcTemplate().query(getBoundSql("ARCHITECTURE_COMMUNITY.SELECT_PAGE_VERSIONS").getSql(),
		pageVersionMapper, new SqlParameterValue(Types.NUMERIC, pageId));
    }

    public List<Integer> getPageVersionIds(long pageId) {
	return getExtendedJdbcTemplate().queryForList(
		getBoundSql("ARCHITECTURE_COMMUNITY.SELECT_PAGE_VERSION_IDS").getSql(), Integer.class,

...
public class JdbcSocialNetworkDao extends ExtendedJdbcDaoSupport implements SocialNetworkDao {
    ...
    public void setSocialAccountPropertyPrimaryColumnName(String socialAccountPropertyPrimaryColumnName) {
	...
	return extendedPropertyDao.getProperties(socialAccountPropertyTableName, socialAccountPropertyPrimaryColumnName,
		accountId);
    }

    public void deleteSocialAccountProperties(long accountId) {
	extendedPropertyDao.deleteProperties(socialAccountPropertyTableName, socialAccountPropertyPrimaryColumnName,
		accountId);
    }

    public void setSocialAccountProperties(long accountId, Map<String, String> props) {
	extendedPropertyDao.updateProperties(socialAccountPropertyTableName, socialAccountPropertyPrimaryColumnName,
		accountId, props);
    }

    public List<Long> getSocialAccountIds(int objectType, String username) {
	return getExtendedJdbcTemplate().queryForList(
		getBoundSql("ARCHITECTURE_COMMUNITY.SELECT_SOCIAL_ACCOUNT_IDS_BY_OBJECT_TYPE_AND_USERNAME").getSql(),
	...
	return getExtendedJdbcTemplate().queryForList(
		getBoundSql("ARCHITECTURE_COMMUNITY.SELECT_SOCIAL_ACCOUNT_IDS_BY_OBJECT_TYPE_AND_OBJECT_ID").getSql(),
		Long.class, new SqlParameterValue(Types.NUMERIC, objectType),
		new SqlParameterValue(Types.NUMERIC, objectId));
    }

    public SocialNetwork getSocialAccountById(long socialAccountId) {
	SocialNetwork account = getExtendedJdbcTemplate().queryForObject(
		getBoundSql("ARCHITECTURE_COMMUNITY.SELECT_SOCIAL_ACCOUNT_BY_ID").getSql(), socialAccountMapper,
		new SqlParameterValue(Types.NUMERIC, socialAccountId));
	...
	getExtendedJdbcTemplate().update(getBoundSql("ARCHITECTURE_COMMUNITY.DELETE_SOCIAL_ACCOUNT_BY_ID").getSql(),
		new SqlParameterValue(Types.NUMERIC, socialNetwork.getSocialAccountId()));
	deleteSocialAccountProperties(socialNetwork.getSocialAccountId());
    }

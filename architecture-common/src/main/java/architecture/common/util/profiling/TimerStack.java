...
public class TimerStack {
    ...
    private static volatile boolean activeFlag = true; // "true".equalsIgnoreCase(System.getProperty(ACTIVATE_PROPERTY,
						       // "false"));

    private static volatile boolean profileMemoryFlag = true; // "true".equalsIgnoreCase(System.getProperty(ACTIVATE_MEMORY_PROPERTY,
							      // "false"));
	...
	if (currentTimer != null && name != null && name.equals(currentTimer.getResource())) {
	    ...
	    printTimes(currentTimer);
	    current.set(null);
	    log.debug(String.format("Unmatched Timer.  Was expecting {}, instead got {}", currentTimer.getResource(),
		    name));
	}
    }

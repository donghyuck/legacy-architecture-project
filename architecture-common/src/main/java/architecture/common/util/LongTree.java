 ...
 * are pointers to other array indices.
 * <p>
 * <p/>
 * The tree holds a maximum of 65534 nodes. It is not intended to be
 * thread-safe. Based on algorithm found in the book
 * "Introduction To Algorithms" by Cormen et all, MIT Press, 1997.
 *
 * @author Matt Tucker
 */
public final class LongTree implements Cacheable, Externalizable {
    ...
    /**
     * Constructor for use with the Externalizable interface. Normal users of
     * this class <b>should not</b> call this constructor.
     */
    public LongTree() {
	// do nothing
    }
	...
	if (parentIndex == 0) {
	    throw new IllegalArgumentException(
		    "Parent key " + parentKey + " not found when adding child " + newKey + ".");
	}
    ...
    /**
     * Recursive method that fills the depthKeys array with all the child keys
     * in the tree in depth first order.
     *
     * @param startIndex
     *            the starting index for the current recursive iteration.
     * @param depthKeys
     *            the array of depth-first keys that is being filled.
     * @param cursor
     *            the current index in the depthKeys array.

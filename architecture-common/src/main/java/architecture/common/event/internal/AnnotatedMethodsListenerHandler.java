...
/**
 * <p>
 * A listener handler that will check for single parameter methods annotated
 * with the given annotation.
 * </p>
 * <p>
 * The default annotation for methods is
 * {@link architecture.common.event.api.EventListener}.
 * </p>
 * 
...
public final class AnnotatedMethodsListenerHandler implements ListenerHandler {
	...
	if (validMethods.isEmpty()) {
	    log.debug(String.format("Couldn't find any valid listener methods on class <{}>",
	...
	if (isAnnotated(method)) {
		...
		throw new RuntimeException((new StringBuilder()).append("Method <").append(method)
			.append("> of class <").append(method.getDeclaringClass()).append("> ")
			.append("is annotated with <").append(annotation.getClass().getName())
			.append("> but has 0 or more than 1 parameters! ")
			.append("Listener methods MUST have 1 and only 1 parameter.").toString());
	    }
	} else {
	    return false;
	}

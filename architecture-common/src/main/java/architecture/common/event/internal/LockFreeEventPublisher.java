...
public class LockFreeEventPublisher implements EventPublisher {
		...
		Iterable<ListenerHandler> build(Object listener) throws IllegalArgumentException {
			...
			if (invokers.isEmpty())
				throw new IllegalArgumentException(
						(new StringBuilder()).append("No listener invokers were found for listener <").append(listener).append(">").toString());
			else
				return invokers;
		}
    ...
    static final class Publisher
    {
        ...
        public void dispatch(Object event)
        {
        	for(ListenerInvoker invoker :getInvokers(event)){
                ...
                {
                    log.error((new StringBuilder()).append("There was an exception thrown trying to dispatch event '").append(event).append("' from the invoker '").append(invoker).append("'.").toString(), t);
                }
        	}
        }
        ...
        Iterable<ListenerInvoker> getInvokers(Object event)
        {
            ...
            return ImmutableSet.copyOf(Iterables.concat(Iterables.transform(allEventTypes, eventClassToInvokersTransformer)));
        }
    ...
    {
        private final ConcurrentMap invokers = (new MapMaker()).makeMap();
	...
	*/
        ...
        private void register(Object listener, ListenerInvoker invoker)
        {
            ...
            {
                Class eventClass;
                for(Iterator iter = invoker.getSupportedEventTypes().iterator(); iter.hasNext(); ((Invokers)invokers.get(eventClass)).add(listener, invoker))
    ...
    {
        invokerBuilder = new InvokerBuilder(((ListenerHandlersConfiguration)Preconditions.checkNotNull(listenerHandlersConfiguration)).getListenerHandlers());

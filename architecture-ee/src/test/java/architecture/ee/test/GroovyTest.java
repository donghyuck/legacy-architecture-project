		...
		try {
			...
			*/
			...
			GroovyCodeSource source = new GroovyCodeSource(
					  new File( "C:/Users/donghyuck/workspace/architecture-ee/src/test/resources/groovy/GroovyCharSetTest.groovy" )
					) ;
			
			groovy.util.GroovyScriptEngine engine = new groovy.util.GroovyScriptEngine( new  java.net.URL[]{ fo.getURL() });
			...
			try {
				/**
				Class groovyClass = engine.getGroovyClassLoader().parseClass(
						source,
						false);
				*/
				Class groovyClass = groovyClassLoader.loadClass(  "groovy.GroovyCharSetTest", true, true);
				//Class groovyClass  = groovyClassLoader.loadClass("groovy.GroovyCharSetTest");
				
								
				//engine.getGroovyClassLoader().setShouldRecompile(true);
				
				System.out.println( 
				engine.getGroovyClassLoader().isShouldRecompile()
				);
...
/*			FileObject fo = VFSUtils.resolveFile( "res://groovy.GroovyCharSetTest" );
			...
			for( Method method : methods ){
				...
				if( method.isAnnotationPresent(Transactional.class )){				
					...
					PlatformTransactionManager transactionManager = ApplicationHelper.getComponent(PlatformTransactionManager.class);

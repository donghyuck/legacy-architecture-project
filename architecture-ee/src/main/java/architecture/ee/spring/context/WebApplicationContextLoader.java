...
public class WebApplicationContextLoader extends ContextLoader {
	...
	for (int i = 0; i < size; i++) {
	    ...
	    log.info((new StringBuilder()).append("Adding '").append(file.getAbsolutePath())
		    .append("' to spring context.").toString());
	    counter++;
	}
	log.info((new StringBuilder()).append("Added ").append(counter).append(" xml files to spring context.")
		.toString());
    }

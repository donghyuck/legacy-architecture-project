...
public class RepositoryImpl extends ComponentImpl implements Repository {
    ...
    public void setServletContext(ServletContext servletContext){
    	...
    	String value = servletContext.getInitParameter( ApplicatioinConstants.ARCHITECTURE_INSTALL_ROOT_ENV_KEY );    
    	if(!StringUtils.isEmpty(value)){
    		try {        		
    			ServletContextResource resource = new ServletContextResource(servletContext, value);          		
        		File file = resource.getFile();
        		log.debug( "Setting install root with " + ApplicatioinConstants.ARCHITECTURE_INSTALL_ROOT_ENV_KEY + ":" + file.getAbsolutePath());  
    	...
    	if(StringUtils.isEmpty(rootURI)){
			...
			try {
				...
				log.debug( "Setting install root with " + ApplicatioinConstants.ARCHITECTURE_INSTALL_ROOT_ENV_KEY + ":" +  obj.getName().getURI()); 
				rootURI = obj.getName().getURI();  
			} catch (FileSystemException e) {
				e.printStackTrace();
			}    		
    ...
    public String getEnvironmentRootPath()
    {
        ...
        String envRootPath = System.getProperty(ApplicatioinConstants.ARCHITECTURE_INSTALL_ROOT_KEY);        
        if(envRootPath == null || "".equals(envRootPath))
            try
            {
            	// architecture_install_root=
            	envRootPath = jndiTemplate.lookup("java:comp/env/" + ApplicatioinConstants.ARCHITECTURE_INSTALL_ROOT_ENV_KEY.toLowerCase(), String.class);                
            }
            catch(Exception e) { }
            
        if(envRootPath == null || "".equals(envRootPath))
        {
            envRootPath = System.getenv(ApplicatioinConstants.ARCHITECTURE_INSTALL_ROOT_KEY);
            if(envRootPath != null && !"".equals(envRootPath))

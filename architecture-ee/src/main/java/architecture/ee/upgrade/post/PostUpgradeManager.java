...
public class PostUpgradeManager {
	...
	if (state == State.RUNNING) {
	    // ApplicationHelper.getAdminService().
	    // //JiveApplication.setApplicationState(ApplicationState.RUNNING,
	    // ApplicationState.POST_UPGRADE_NEEDED);
	} else {
	    log.warn((new StringBuilder()).append("Unable to initialize post upgrade, invalid application state '")
		    .append(state).append("'.").toString());
	}
    }

    public void upgradeInProgress() {
	// JiveApplication.setApplicationState(ApplicationState.POST_UPGRADE_NEEDED,
	// ApplicationState.POST_UPGRADE_STARTED);
    }

    public void upgradeComplete() {
	if (getUpgradeTasks().size() <= 0) {
	    // JiveApplication.setApplicationState(ApplicationState.POST_UPGRADE_STARTED,
	    // ApplicationState.POST_UPGRADE_COMPLETE);
	    // JiveApplication.setApplicationState(ApplicationState.POST_UPGRADE_COMPLETE,
	    // ApplicationState.RUNNING);
	    if (ApplicationHelper.getState() != State.RUNNING)
		throw new IllegalStateException(
			"Post upgrade manager: Failed to transition application to running state");
	}
    }
	...
	try {
		...
		if (config == null) {
		    ...
		    if (!config.validate())
			log.error(
				"com/jivesoftware/community/upgrade/post/post-upgrade.xml' is not configured correctly.");
		    else

...
public class JdbcI18nLocaleDao extends ExtendedJdbcDaoSupport implements I18nLocaleDao {
	...
	return getExtendedJdbcTemplate().query(getBoundSql("ARCHITECTURE_FRAMEWORK.SELECT_ALL_LOCALE").getSql(),
		new I18nLocaleRowMapper());
    }

    public I18nLocale getI18nLocaleById(long localeId) {
	return getExtendedJdbcTemplate().queryForObject(
		getBoundSql("ARCHITECTURE_FRAMEWORK.SELECT_LOCALE_BY_ID").getSql(), new Object[] { localeId },
		new int[] { Types.INTEGER }, new I18nLocaleRowMapper());
    }
	...
	return getExtendedJdbcTemplate().queryForObject(
		getBoundSql("ARCHITECTURE_FRAMEWORK.SELECT_LOCALE_BY_CODE").getSql(),
		new Object[] { locale.getLanguage(), locale.getCountry() }, new int[] { Types.VARCHAR, Types.VARCHAR },
		new I18nLocaleRowMapper());
    }
	...
	getExtendedJdbcTemplate().update(getBoundSql("ARCHITECTURE_FRAMEWORK.INSERT_LOCALE").getSql(),
		new Object[] { impl.getLocaleId(), impl.getLanguage(), impl.getCountry(), impl.getVariant(),
			// impl.getEncoding(),
			new java.sql.Date(now), new java.sql.Date(now) },
	...
	getExtendedJdbcTemplate().update(getBoundSql("ARCHITECTURE_FRAMEWORK.DELETE_LOCALE_BY_ID").getSql(),
		new Object[] { locale.getLocaleId() }, new int[] { Types.INTEGER });
    }

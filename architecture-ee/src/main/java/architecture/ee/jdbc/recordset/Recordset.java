...
/**
 * Core-level framework class: Recordset - General purpose, tabular data
 * structure. <br>
 * <br>
 * Represents a disconnected recordset, a kind of multi-purpose ResultSet
 * without connection to the database, much like the famous ADO disconnected
 * recordset from VB6, can be created from a SQL query or manufactured by code
 * to represent any tabular data. It is a kind of generic data container that
 * may save a lot of work because it can be used to represent several entities
 * avoiding the need to write custom classes to represent each specific entity.
 * It is serializable, so it can be safely used in HTTP Sessions, JNDI, etc.
 * <br>
 * <br>
 * This recordset mantains all data in memory and does not need an open
 * connection to the database, provides several constructors to isolate the
 * programmer from the task of programming with the JDBC API, but also provides
 * low level access if desired. <br>
 * <br>
 * Each field in the recordset is mantained in its native data type as
 * represented by the JDBC driver, or a java null value. <br>
 * <br>
...
public class Recordset implements Serializable {
    ...
    /**
     * Create a recordset with one record containing general recordset
     * parameters like: recordcount, pagecount, currentpage
     * 
	...
	// create an identical recordset containing only the records for the
	// requested page
	...
	if (value != null) {
		...
		if (!(value instanceof java.util.Date))
		    throw new RecordsetException(
			    "Invalid data type of field: " + fieldName + "; passed value must be a DATE object.");
		break;

	    case java.sql.Types.INTEGER:
		if (!(value instanceof java.lang.Integer) && !(value instanceof java.lang.Long))
		    throw new RecordsetException(
			    "Invalid data type of field: " + fieldName + "; passed value must be an INTEGER object.");
		break;
	    // parche 2010-07-07
	    case java.sql.Types.DOUBLE:
		if (!(value instanceof java.lang.Double) && !(value instanceof java.math.BigDecimal))
		    throw new RecordsetException(
			    "Invalid data type of field: " + fieldName + "; passed value must be an DOUBLE object.");
		break;
	    }
	}
     ...
     *            SQL Query that returns a Resultset
     * @param limit
     *            Maximum number of rows to read from the DataBase
     * @throws Throwable
     */
    ...
    /**
     * Default constructor used when creating the recordset from code (a
    ...
    /**
     * Returns a text-formatted report with the structure of the recordset, very
    ...
    /**
     * Copy record values from this recordset to a destination recordset, using
     * the current record of both recordsets. Destination recordset fields that
     * match names with source recordset fields must be of the same type too. It
     * does not matter if some fields from the source recordset are not defined
     * in the destination recordset. Only name-matching fields will be
     * considered for the operation.
     * 
     * @param rs
     *            Destination recordsets
     * @throws Throwable
     */
	...
	if (recNum < 0 || recNum > _data.size() - 1) {
	    ...
	    if (recNum == -1)
		errMsg.append(
			"After creating a Recordset you must move to a valid record using next(), first(), last() or setRecordNumber() methods before attempting read/write operations with any record of this Recordset; ");
	    errMsg.append("This Recordset contains " + _data.size()
     ...
     * 
     * @param col
     *            Name of the column to be used for the sort
     * @throws Throwable
     */
    ...
    /**
     * Retrieve recordset metadata. This method returns a Recordset containing
     * the columns: name, typename, typeid. "typeid" is the java.sql.Type value
     * <br>
    ...
    /**
     * Sumar o totalizar el valor de una columna para todos los registros del
     * recordset
     * 
     * @param colName
     *            Nombre de la columna a sumar
